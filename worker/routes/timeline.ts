...
timelineRoutes.get('/', async (c) => {
  ...
  let feedQ: string, diaperQ: string, sleepQ: string, tempQ: string, medQ: string;
  let binds: string[];
  ...
  if (from && to) {
    ...
    medQ = "SELECT id, time, medication_name, dosage, unit, note, 'medication' as record_type FROM medication_records WHERE time >= ? AND time <= ? ORDER BY time DESC";
    binds = [from, to];
  } else {
    ...
    medQ = "SELECT id, time, medication_name, dosage, unit, note, 'medication' as record_type FROM medication_records WHERE date(time) = ? ORDER BY time DESC";
    binds = [date];
  }

  const [feeds, diapers, sleeps, temps, meds] = await Promise.all([
    ...
    db.prepare(medQ).bind(...binds).all().catch(() => ({ results: [] })),
  ]);
